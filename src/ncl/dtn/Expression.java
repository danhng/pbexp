...
import com.sun.scenario.effect.impl.prism.ps.PPSBlend_EXCLUSIONPeer;
...
/**
 * WARN! Operator objects cannot be reused. It is recommended that {@link Operator#get(int)} method is used to get a new
 * instance of some operator. Operator reuses might cause erroneous behaviours.
 *
...
public class Expression {
    ...
    public static class Parenthesis implements Expressible {
        ...
        public Evaluable toEvaluable() throws PBEXPException {
            throw new PBEXPException(PBEXPException.CLASS_CAST_EXCEPTION, "Parenthesis class does not implement Evaluable.");
        }
     ...
     */
     Evaluable compute_internal() throws PBEXPException {
        ...
        while (!isResolved()) {
            ...
            Debug.info("**********Current Expressible: %s[%d]\n", current.toRep(), internals_mutated.indexOf(current));
            switch (current.toCategory()) {
                case PARENTHESIS_OPEN: case PARENTHESIS_CLOSE: {
                    // advance and wrap around (this is part of the algorithm). Number of repeating rounds not known at the start
                ...
                case OPERAND: {
                    ...
                    for (int i = 0; i < neighbours.size(); i++) {
                        ...
                        //todo caching this could help in performance
                                ...
                                nIndex, index))
                        {
                            Debug.debug("Operator %s{%d}[%d] cannot be associate-able with Operand %s[%d]. Removing from promising list\n",
                    ...
                    try {
                        switch (neighbours.size()) {
                            ...
                            default:
                                throw new IllegalArgumentException("Something wrong with Expression at " + internals_mutated.indexOf(current) + ": " + neighbours.size() + " neighbours");
                        }
                    ...
                    if (operationWinner.checkOperandAssociability(current.toEvaluable())) {
                        ...
                        Debug.warn("Operand %s[%d] cannot be associated to Operation %s. Ignored.\n",
                ...
                case OPERATOR: {
                    ...
                    if (current.toOperator().getOperandsCount() == Operator.BINARY_COUNT) {
                        ...
                        for (Expressible operator : neighbours) {
                            if (operator.toCategory() == Expressible.Category.OPERATOR && operator.toOperator().getOperandsCount() == Operator.BINARY_COUNT)
                                throw new PBEXPException(PBEXPException.BINARY_OPERATORS_NEIGHBOURING, "Operator " + current + "[" + index + "]");
    ...
    private static List<Evaluable> getOperands(LinkedList<Expressible> e) {
        return e.stream().filter(t -> t.toCategory() == Expressible.Category.OPERAND).map(Expressible::toEvaluable).collect(Collectors.toList());
    }
    ...
    private void simplify(int operationIndex) {
        ...
        if (isOutOfBound(operationIndex)) {
            Debug.warn("Operation Index %s is out of bound: %d\n", operationIndex, internals_mutated.size());
            return;
        }
    ...
    private Expression addExpressible(Expressible e) {
        if (internals_mutated.contains(e)) {
            Debug.warn("Expressible %s already in the expression. Proceed with caution.\n", e.toRep());
        }
        ...
        // raise priority if Expressible is Operator
